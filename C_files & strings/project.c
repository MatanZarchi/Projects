...
void createFile(char* a) //תרגיל 3 סעיף א 
{
...
char commonestLetter(char* filename) //תרגיל 3 סעיף ב 
{
	createFile(filename);
	int winner, flag = 0;;
	int check = 0, i = 0;
...
int string_with_no_spaces_size(char* a) //תרגיל 2- מוחק רווחים ומספרים 
{
	int i = 0, counter = 0;
	while (a[i] != '\0')
	{
...
char* clearDigitsAndSpaces(char* a) //תרגיל 2 פונקציה 
{
...
int colom_for_string(char* a, int index) //תרגיל 1- סופר אורך לכל שורה
{
...
int row_for_string(char* a, char letter) //תרגיל 1- פונקציה לספירת שורות
{
	int i = 0;
	int counter = 0;
	while (a[i] != '\0')
...
char** split(char* a, char letter, int* counter, int* r) //תרגיל 1
{
	...
	*counter = colom;
	...
	while (a[i] != '\0')
	{
		...
		if ((letter == tolower(a[i]) || letter == toupper(a[i])) && openingOfWord == true)
		{
			...
			if (colom > * counter) // משתנה של גודל המערך 
			{
				*counter = colom;
			}
...
int countDeclarations(char* declaration) // תרגיל 5-כמה הצהרות יש
{
	int count = 0, i = 0;
	while (declaration[i] != ';')
	{
...
void printing_file(FILE* f, char* filename) // תרגיל 5
{
	f = fopen(filename, "r");
	int ch = 0;
	while (ch != EOF)
...
char* casting_from_int_to_string(int sum) // תרגיל 5 ממיר את משתנה של מספר למחרוזת 
{
	int i = 0, j = 0;
	char cast = 0;
...
int checkType(char* declaration, int* index) // תרגיל 5-בודק סוגים של משתנים(טייפים
{
...
void memoryReport(char* declaration, char* filename) // תרגיל 5
{
	...
	for (matrix_row = 0; matrix_row < row; matrix_row++) // הצהרות
